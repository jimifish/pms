    ...
    @try {
        ...
        @try {
            NSURLSessionDownloadTask *downloadTask = [manager downloadTaskWithRequest:request progress:nil destination:^NSURL *(NSURL *targetPath, NSURLResponse *response) {
                ...
                NSString* downloadFilePath = [NSString stringWithFormat:@"Documents/%@", self.file.name];
                NSString *fPath=[NSHomeDirectory() stringByAppendingPathComponent:downloadFilePath];
                NSLog(@"fPath: %@", fPath);
                fPath = [fPath stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
